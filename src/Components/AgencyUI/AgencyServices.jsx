...
const AgencyServices = ({ agency_services }) => {
    ...
    const selectedCategory = categoryList?.find((item) => item._id === category) || {};
    const selectedSubCategory = subCategoryList?.find((item) => item._id === subCategory) || {};
    ...
    return (
        <VStack alignItems="flex-start" gap={5} width="95%">
            ...
            {selectedCategory && (
                <HStack className="border p-4 rounded-md" width="100%" justifyContent="space-between">
                    <Text marginBottom="0" fontSize="1.2rem">{selectedCategory.category_name} / {selectedSubCategory.sub_category_name}</Text>
                    <HStack>
